...
from .models import Post, Category, Tag, Comment, Photo, Like, Post_photo
...
# 사용자는 블로그 게시물에 사진을 업로드할 수 있어야 합니다.  -- 있음
# 사용자는 게시물의 사진에 '좋아요'를 누를 수 있어야 합니다.
# 사용자는 자신이 '좋아요'를 누른 사진들을 모아서 볼 수 있어야 합니다.
# tests.py 파일에 테스트 코드를 작성
# class PhotoLikeTest(TestCase):
#     def setUp(self):
#         self.user = User.objects.create_user(username='testuser', password='12345')
#         self.post = Post_photo.objects.create(title='Test Post', content='Test Content', author=self.user)
#         self.photo = Photo.objects.create(post=self.post, image='/static/blog/images/amsterdam_sky_sunflower.jpg')

#     def test_user_can_like_photo(self):
#         self.client.login(username='testuser', password='12345')
#         response = self.client.post(f'/like/{self.photo.id}/')
#         self.assertEqual(response.status_code, 200)
#         self.assertTrue(Like.objects.filter(user=self.user, photo=self.photo).exists())

#     def test_user_can_view_liked_photos(self):
#         Like.objects.create(user=self.user, photo=self.photo)
#         self.client.login(username='testuser', password='12345')
#         response = self.client.get('/liked-photos/')
#         self.assertEqual(response.status_code, 200)
#         self.assertContains(response, '/static/blog/images/amsterdam_sky_sunflower.jpg')
...
class PostLikeTestCase(TestCase):
    ...
    def test_like_post(self):
        ...
        # 좋아요 추가
        response = self.client.post(f'/blog/post/{self.post.id}/like/')
        self.assertEqual(response.status_code, 302)  # 리다이렉션 확인
        self.assertTrue(self.user1 in self.post.likes.all())  # 좋아요 추가 확인

        # 좋아요 제거
        response = self.client.post(f'/blog/post/{self.post.id}/like/')
        self.assertEqual(response.status_code, 302)  # 리다이렉션 확인
        self.assertFalse(self.user1 in self.post.likes.all())  # 좋아요 제거 확인
    ...
    def test_like_post_by_multiple_users(self):
        ...
        self.client.post(reverse('like_post', args=[self.post.id]))
        self.post.refresh_from_db()
        self.assertTrue(self.user2 in self.post.likes.all())
